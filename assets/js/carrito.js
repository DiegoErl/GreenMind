  ...
  if (localStorage.getItem("listaDeseo") == null) {
    ...
    for (let i = 0; i < listaExiste.length; i++) {
      if (listaExiste[i]["idProducto"] == idProducto) {
        Swal.fire({
          title: "Aviso",
          text: "EL PRODUCTO YA FUE AGREGADO EN TU LISTA DE DESEOS",
          icon: "warning",
        });
...
function agregarCarrito(idProducto, cantidad, accion = false) {
  if (localStorage.getItem("listaCarrito") == null) {
    ...
    for (let i = 0; i < listaExiste.length; i++) {
      ...
      if (listaExiste[i]["idProducto"] == idProducto) {
        Swal.fire({
          title: "Aviso",
          text: "EL PRODUCTO YA FUE AGREGADO EN TU CARRITO DE COMPRAS",
          icon: "warning",
        });
  ...
  Swal.fire({
    title: "Aviso",
    text: "PRODUCTO AGREGADO A TU CARRITO DE COMPRASS",
    icon: "success",
  });
  cantidadCarrito();
}
